...
### ✅ Проблема №4: Burn message body не создается (РЕШЕНО 2025-10-10)
...
**Результат:** Burn message body создается корректно

---

### 🔄 Проблема №5: Swap payload/body и курс обмена (В ПРОЦЕССЕ 2025-10-10)

**Статус:** 🔄 ИСПРАВЛЯЕТСЯ

**Проблема:** 
1. Router v2.2 возвращает `body` вместо `payload` - swap отправлялся без тела сообщения
2. Курс обмена неправильный из-за перепутанных резервов
3. Баланс jettons проверялся неправильно (не сравнивался ДО/ПОСЛЕ)

**Лог ошибки:**
```
"allKeys": ["to", "value", "body"]
"payload": "null"
Body size: 0 bits  ← Пустое тело!
Expected: 898 jettons ← Неправильно, должно быть ~10,000
```

**Решение:**
1. **Swap body:** Используем `swapTxParams.body || swapTxParams.payload`
2. **Курс обмена:** Исправлена логика определения резервов:
   - reserve0 (большее число) = RUBLE jettons
   - reserve1 (меньшее число) = TON
3. **Баланс проверка:** Сравниваем баланс ДО и ПОСЛЕ swap

**Ожидаемый результат:** 
- Swap с корректным телом сообщения
- Курс ~104,000 RUBLE/TON (0.1 TON = ~10,400 RUBLE)
- Корректный расчет полученных jettons
